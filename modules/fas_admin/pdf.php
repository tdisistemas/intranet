...
<?php
_bienvenido_mysql();
$cedula_emp=$_POST["cedula"];
$estatus= _dameestatus($cedula_emp);
if($estatus!=1){
    
       alerta('La cédula suministrada ('.$cedula_emp.') no a registrado ninguna información dentro del FAS METRO');
	ir("../../dashboard.php?data=fas_admin");
} 
$fecha_ingreso = _dametipoempyfechainicio($cedula_emp, 2);
$sqlq = mysql_query("SELECT * FROM `datos_empleado_rrhh` d left join datos_empleado_titulo t on(d.titulo=t.tipo) left join datos_empleado_profesion p on (d.profesion=p.tipo_p) left join datos_empleado_estado e on (e.estado=d.estado) left join datos_empleado_municipio m on (d.municipio=m.municipio) left join fas_adicional s on(s.cedula_empleado=d.cedula) where d.cedula=$cedula_emp ");
...
<body style="overflow: scroll;">  
    <div class="contenedor" >
        <div class="header" >
            ...
            <img class="img-empleado" style="width: 70px important ;" src="../../src/images/FOTOS/<?php echo $cedula_emp; ?>.jpg" /> 
            ...
            <div class="cuerpo" >
                ...
                <table border=1>
                    ...
                    <tr>
                        ...
                        <td colspan="4">
                            Apellidos: <?php echo $row["apellido"] ?>
                        </td>
                    </tr>
                    <tr>
                        <td >
                            C.I:  <?php echo $row["cedula"] ?>
                        </td>
                        <td >
...
$sql = mysql_query("SELECT * FROM `datos_familiares_rrhh` d left join fas f on(d.cedula_empleado=f.cedula_empleado) WHERE d.cedula_familiar=f.cedula_familiar and d.cedula_empleado=$cedula_emp and d.parentesco!='titular'  ");
...
while ($row_cont = mysql_fetch_array($sql)) {
    ?>
                        ...
                        <tr class="gradeA">
                            ...
                            <td><?php if ($row_cont["parentesco"]=='Otro(a)'){echo 'No FAS';}else{ echo $row_cont["parentesco"];} ?></td>
