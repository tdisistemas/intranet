...
if (!$_GET["flag"]){ir('dashboard.php?data=prueba');}
?>

<div id="contentHeader">
<h2>Eliminar Usuario - <?php echo 'ID: ' . strip_tags($_GET["id"]); ?></h2>
</div> <!-- #contentHeader -->	
...
if($_GET["flag"]){
  ...
  _bienvenido_mysql();
  $sql="UPDATE Prueba_User SET status = 1 WHERE idUser = ". $idmi;
	$result = mysql_query($sql) or die('Error Eliminando Usuario - ' . mysql_error());
	if($result){
		notificar("Usuario eliminado con exito", "dashboard.php?data=prueba", "notify-error");
	}
	else {
		die(mysql_error());
	}			
